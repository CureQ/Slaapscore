# Bestandsnaam: ECG_results_android_gui.py
...
def get_participant_info(ecg_file_name, preprocessed_df):
    try:
        # Zoek het deelnemernummer uit het bestandpad (bijvoorbeeld: "MoveSense_participant_35")
        match = re.search(r'participant[_\-]?(\d+)', ecg_file_name)
        if match:
            # Haal het nummer uit de gevonden match en zet het om naar een integer
            participant_number = int(match.group(1))
            print("Measurement of participant {0}".format(participant_number))
        else:
            participant_number = None
            # Geef een foutmelding als het deelnemernummer niet gevonden wordt in het pad
            raise ValueError("Deelnemernummer kon niet worden gevonden in pad: {0}".format(ecg_file_name))
    except Exception as e:
        # Geef een foutmelding wanneer er iets misgaat bij het bepalen van het participantnummer
        print(f"Fout bij het bepalen van participant number: {e}")
        participant_number = None

    # Haal de bestandsnaam uit het volledige pad, los van het besturingssysteem
    file_name = ecg_file_name.split("/")[-1] if "/" in ecg_file_name else ecg_file_name.split("\\")[-1]

    try:
        # Haal de startdatum uit de bestandsnaam door te splitsen op 'T'
        start_date_time = file_name.split("T")

        # Pak de laatset drie delen van de datum 
        start_date = start_date_time[0].split("-")[-3:]

        # Voeg de datumdelen samen tot een string in het formaat YYYY-MM-DD
        start_date = "-".join(start_date)

        print("Measurement start date:", start_date)

        # Haal de starttijd uit de bestandsnaam, verwijderd de extensie en vervang '_' met ':'
        start_time = start_date_time[1].split(".")[0].replace("_", ":")
        print("Measurement start time:", start_time)

        # Combineer datum en tijd tot een datetime-object voor een nauwkeurige tijdsaanduiding
        start_timestamp = datetime.strptime(f"{start_date} {start_time}", "%Y-%m-%d %H:%M:%S")
        print(start_timestamp)

    except Exception as e:
        # Geef een foutmelding als de starttijd niet uit de bestandsnaam gehaald kan worden
        print("Start timestamp could not be extracted.")
        # Uitleg over het verwachte bestandsformaat om gebruikers te helpen het probleem te begrijpen
        print("Expected file name should end like: xxxxxxxxTxxxxxxZ_xxxxxxxxxxxx_ecg_stream.csv")
        # Toon de daadwerkelijke bestandsnaam die het probleem veroorzaakt
        print("Your file name looks like:", file_name)
        # Toon de exacte foutmelding voor debugging
        print("Fout:", e)
        # Zet de starttijd op None, omdat deze niet bepaald kon worden
        start_timestamp = None

    # Werk met de preprocessed dataframe
    df = preprocessed_df
    # Pak de eerste timestamp uit het dataframe
    first_timestamp = df["timestamp"].iloc[0]
    # Pak de laatste timestamp uit het dataframe
    last_timestamp = df["timestamp"].iloc[-1]
    # Bereken de tijdsduur in milliseconden tussen de eerste en laatste meting
    duration_milliseconds = (last_timestamp - first_timestamp) * 1000
    # zet de tijdsduur om naar seconden (int)
    duration_seconds = int(duration_milliseconds / 1000)
    # Bereken minuten en seconden uit het totaal aantal seconden
    minutes, seconds = divmod(duration_seconds, 60)
    # Bereken uren en minuten uit het totaal aantal minuten
    hours, minutes = divmod(minutes, 60)

    # Print de totale duur van de meting, in uren, minuten en seconden
    print("Measurement duration: {} hours, {} minutes, {} seconds.".format(hours, minutes, seconds))
    ...
    # Print de starttijd van de meting
    print("Measurement started on {0}".format(start_timestamp))
    # Print de eindtijd van de meting
    print("Measurement  ended  on {0}".format(end_timestamp))

    # Geef het participantnummer, de starttijd en eindtijd terug
    return participant_number, start_timestamp, end_timestamp


# Functie om alle benodigde resultaten uit het HDF5-resultatenbestand te halen
def get_results(results_file, show_plots=False):
    # Open HDF5 file, in leesmodus (r=read)
    with h5py.File(results_file, 'r') as results:
        # Lees de confusion matrix uit het bestand
        confusions = results["confusions"][()]
        print(confusions)

    # Pak het aantal voorspellingen per label uit de eerste rij van de matrix
    predicted_label_amount = confusions[0][0]
    predicted_label_amount

    # Print het aantal voorspelingen per slaapstadium
    ...
    # Open opnieuw het HDF5 bestand om voorspellingen te lezen 
    with h5py.File(results_file, 'r') as results:
    ...
    if show_plots:
        plt.close()
        # Visualiseer slaapstadia per 30 seconden
